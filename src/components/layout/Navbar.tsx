...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-sm">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map(link => (
            <Link 
              key={link.path} 
              to={link.path} 
              className={`text-sm font-medium transition-all duration-200 hover:text-sbs-orange relative ${
            ...
            >
              {link.name}
              {location.pathname === link.path && (
                <div className="absolute -bottom-1 left-0 right-0 h-0.5 bg-sbs-orange rounded-full" />
              )}
            </Link>
          ))}
      ...
      {/* Mobile menu */}
      {isMenuOpen && (
        <div className="container md:hidden py-6 px-4 border-t border-border/40 bg-background/95 backdrop-blur">
